        ...
        <div class="hero-content">
            <div class="scan-module">
                ...
                <div class="status-placeholder">
                    <?php
                    // on verifie si le scan est fini et si le PDF a bien ete cree
                    if (isset($_GET['scan']) && $_GET['scan'] == 'termine' && file_exists('rapport.pdf')) {
                        
                        $cible_affichee = htmlspecialchars($_GET['cible']);

                        // affichage propre
                        echo "<div style='background: white; padding: 20px; border-radius: 8px; border-left: 5px solid var(--primary-orange); box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>";
                        echo "<h3 style='margin-top: 0; color: var(--text-dark);'>✅ Diagnostic terminé !</h3>";
                        echo "<p style='margin-bottom: 15px;'>L'audit en boîte noire de <strong>" . $cible_affichee . "</strong> a été généré avec succès.</p>";
                        
                        // le bouton pointe desormais vers rapport.pdf !
                        echo "<a href='rapport.pdf' download='Audit_Cyber_" . $cible_affichee . ".pdf' style='display: inline-block; padding: 10px 15px; background-color: var(--text-dark); color: white; text-decoration: none; border-radius: 5px; font-weight: bold;'>📄 Télécharger le Rapport (PDF)</a>";
                        echo "</div>";

                    } else {
                        // texte par defaut
                        echo "<em>Cible : En attente d'analyse...</em>";
                    }
                    ?>
                </div>
                </div>
