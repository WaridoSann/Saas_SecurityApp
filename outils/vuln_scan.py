...
def faire_nikto(cible_url, mode_scan):
    ...
    try:
        ...
        if not failles_trouvees:
            if mode_scan == "simple":
                return "Le diagnostic rapide de surface n'a détecté aucune mauvaise configuration évidente sur vos serveurs."
            else:
                return "Nous avons lancé des attaques approfondies sur vos fondations. Aucune faille de configuration majeure n'a été détectée dans le temps imparti."
